...
export default {
  ...
  methods:{
    ...
    async onConfirmRemove(prefixToRemove){
      const res = 
      await fetch('http://localhost:8088/rdf4j-server/repositories/'+this.$route.params.repo+'/namespaces/'+prefixToRemove,
    ...
    async closeCreateModal(){
      ...
      if(this.newNSname !== '' && this.newNSprefix !== '' && this.newNSname.match(/(http(s){0,1}:\/\/)\w+/g)){
        ...
        if(this.tableData.data.some(item => item.prefix === this.newNSprefix || item.namespace === this.newNSname)){
          ...
          await fetch('http://localhost:8088/rdf4j-server/repositories/'+this.$route.params.repo+'/namespaces/'+this.newNSprefix,
          {
            method: 'PUT',
            headers:{
    ...
    async closeEditModal(){
      ...
      if(this.editedNSname !== '' && this.editedNSname.match(/(http(s){0,1}:\/\/)\w+/g) && !exists){
          ...
          await fetch('http://localhost:8088/rdf4j-server/repositories/'+this.$route.params.repo+'/namespaces/'+this.prefixEditNS,
          {
            method: 'PUT',
            headers:{
              'Content-Type': 'text/plain'
    ...
    async queryAllNamespaces(){
      this.allNamespaces = await fetch('http://localhost:8088/rdf4j-server/repositories/'+this.$route.params.repo+'/namespaces', {
          method: 'GET',
          headers: {
